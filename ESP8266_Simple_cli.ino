...
void showHelp() {
  ...
  Serial.println("");
  Serial.println("**********************************************");
  Serial.println("|SysConfig Wlan: wlanset|{SSID}|{Password}");
  Serial.println("|SysConfig Mqtt: mqttset|server|port|clientId|{username}|{Password}");
  Serial.println("|Print Wlan Info:wlan                        |");
  Serial.println("|Test Buzzer {n} times: beep {n}             |");
  Serial.println("|Print tempature:temp                        |");
  Serial.println("|Print humiduty:hum                          |");
  Serial.println("**********************************************");
}
/**
   打印温度
...
/**
   发送HTTP数据到服务器
*/
void postToHttpServer(JSONVar json) {
